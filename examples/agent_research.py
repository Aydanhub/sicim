"""LLM agent workflow'u: paralel araç çağrıları, insan onayı, çökmeden devam.

Akış: plan (LLM) -> 3 paralel arama -> taslak (LLM) -> insan onayı bekle -> yanıt.

Demo, agent insan onayı beklerken süreci "çökertir". Yeni süreç run'ı devralır,
onay sinyali gelince tamamlar — ve her benzersiz LLM çağrısı yalnızca BİR kez
yapılmıştır: tamamlanmış çağrılar journal'dan replay edilir, asla tekrarlanmaz.
...
async def research_agent(ctx: sicim.WorkflowContext, question: str):
    ...
    # Paralel araç çağrıları: op kimlikleri çağrı anında atanır, replay kararlıdır.
    sources = await ctx.gather(
        *(ctx.step(web_search, q, name=f"search-{i}") for i, q in enumerate(plan["queries"]))
    )
...
async def main():
    ...
    await rt1.shutdown()
    print(f"  plan + 3 arama + taslak bitti, onay beklerken süreç ÇÖKTÜ.")
    ...
    print(f"  kaynaklar: {[s['top_result'] for s in result['sources']]}")
    print(f"  LLM çağrı sayıları: {llm_calls}")
