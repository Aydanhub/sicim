...
* ``every=N``  — every N seconds, aligned to the previous fire time;
...
* ``cron="…"`` — classic five-field cron (``minute hour day-of-month month
  ...
  ``@hourly``-style aliases. Evaluated in UTC unless an IANA ``tz`` is given.
...
class CronExpr:
    ...
    def next_after(self, ts: float, tz: dt.tzinfo) -> float | None:
        ...
        current = dt.datetime.fromtimestamp(ts, tz).replace(second=0, microsecond=0)
                ...
                for hour in hours:
                    ...
                    for minute in minutes:
                        if hour == current.hour and minute < current.minute:
                            continue
                        candidate = current.replace(hour=hour, minute=minute).timestamp()
                        if candidate > ts:  # DST folds can map a wall time backwards
                            return candidate
            current = (current + dt.timedelta(days=1)).replace(hour=0, minute=0)
        return None
...
def build_spec(
    *,
    ...
    if every is not None:
        seconds = every.total_seconds() if isinstance(every, dt.timedelta) else float(every)
        if not seconds > 0:
    ...
    return Cron(CronExpr.parse(cron), tz)  # type: ignore[arg-type]
